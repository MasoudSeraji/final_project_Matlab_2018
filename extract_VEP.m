...
% event ocured  to extract Visual Evoked Potential for each channel
% first the function exttract the 313 samples after each event and then 
% the output will be exttracted with averaging 
...
% for example if you ha 32 channel the number of third dimention will be 32

epoch_occiptal_channel=zeros(15,313,9);%preallocating
% extract channel 24 until 32 (Occipital channels)
 for i=24:32
     epoch=zeros(15,313);%preallocating for epochs
for m=1:15
    % for each event we extract 313 samples after event
     epoch(m,:)= EEG_data(i,(event_samples(1,m):event_samples(1,m)+312));
